...
soft_actor_critic_model_entropy_activation_function = soft_value_based_model_entropy_activation_function


class SoftActorCriticDGNAgent(ActorCriticDGNAgent):
    def __init__(self, in_dim, act_dim):
        super(SoftActorCriticDGNAgent, self).__init__(in_dim, act_dim)
    ...
    def cal_q_loss(self, sample, losses, log_vars=None, global_steps=None):
        ...
        with torch.no_grad():
            ...
            next_probs, next_log_probs = self.actor_learned_model(next_obs, next_adj, return_log_pi=True)
            v_values, _ = self.critic_target_model(next_obs, next_adj)
