...
import torch
import numpy as np
from trainers.actor_critic_trainer import ActorCriticTrainer
...
class SoftActorCriticTrainer(ActorCriticTrainer):
    """
    ...
    def i_iter_dict(self):
        return {'i_actor': self.i_iter_actor, 'i_critic': self.i_iter_critic, 
                'i_alpha': self.i_iter_alpha, 'i_episode': self.i_episode}
    ...
    def _training_step(self, log_vars):
        ...
        for _ in range(hparams['training_times']):
            ...
            log_vars['Training/q_grad'] = (self.i_iter_critic, get_grad_norm(self.agent.alpha, l=2))
